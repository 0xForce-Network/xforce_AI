...
xforce_supervisor_main() {
  ...
  mkdir -p "$XFORCE_SUPERVISOR_STATE_DIR/logs" "$XFORCE_SERVICE_LOG_DIR/portal-backend" "$XFORCE_SERVICE_LOG_DIR/example-pty-service" "${XFORCE_BOOT_STATE_DIR:-/tmp/xforce-ai}/portal" /tmp/xforce-ai/pty /tmp/xforce-ai/metrics
