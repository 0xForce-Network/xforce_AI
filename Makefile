...
build-all:
	@IFS=','; for variant in $(VARIANTS); do \
		IMAGE_REGISTRY=$(IMAGE_REGISTRY) IMAGE_NAME=$(IMAGE_NAME) IMAGE_TAG=$(IMAGE_TAG) PLATFORMS=$(PLATFORMS) PUSH=$(PUSH) ENABLE_NUSHELL=$(ENABLE_NUSHELL) ENABLE_CLOUDFLARED=$(ENABLE_CLOUDFLARED) ENABLE_FASTSOURCES=$(ENABLE_FASTSOURCES) scripts/build-image.sh $$variant; \
	done
